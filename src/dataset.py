...
def mass_target_info(slide: str, targets: list[dict]) -> list[list]:
    ...
    for target in targets:
        if len(target["labels"]) != 1:
            utils.log(
                "WARN",
                f"'{slide}' target skipped: no or multiple labels: {target['labels']}",
            )
            continue
        ...
        match target["labels"][0]:
            ...
            case _:
                utils.log(
                    "WARN",
                    f"'{slide}' target skipped: unknown label: '{target['labels'][0]}'",
                )
                continue
        ...
        match target["tool"]:
            ...
            case "rectangle":
                bbox = pts_to_bb(segments)
            case None:
                utils.log("NOTICE", f"'{slide}' target: no tool specified")
                bbox = pts_to_bb(segments)
            case _:
                utils.log(
                    "WARN",
                    f"'{slide}' target skipped: unknown tool: '{target['tool']}'",
                )
                continue
        ...
        for field in target.keys():
            if field not in expect_fields:
                utils.log(
                    "NOTICE",
                    f"'{slide}' target: unknown field: '{field}': '{target[field]}'",
                )

        uid = utils.mass_hash(slide, bbox)
        info_df.append([uid, slide, tag, bbox[0], bbox[1], bbox[2], bbox[3]])
...
def get_mass_info(labels: list[dict]) -> pd.DataFrame:
    ...
    for label in tqdm(labels):
        ...
        if label["description"] is not None and label["description"] != "":
            utils.log(
                "NOTICE", f"'{slide}': ignore description: '{label['description']}'"
            )

        if label["note"] is not None and label["note"] != "":
            utils.log("NOTICE", f"'{slide}': ignore note: '{label['note']}'")

        tags = label["sub_tags"]
        if len(tags) > 1 or (len(tags) == 1 and tags[0] != "done"):
            utils.log("NOTICE", f"'{slide}': ignore tags: '{tags}'")
    ...
    info_df = pd.DataFrame(
        info_df,
        columns=["uid", "slide", "tag", "x_min", "y_min", "width", "height"],  # type: ignore
    )
...
def slide_info(path: Path) -> list | None:
    ...
    try:
        image = slide.read_region((0, 0), 4, slide.level_dimensions[4])  # type: ignore
    except Exception as e:
...
def get_slide_info(path_list: list[Path]) -> pd.DataFrame:
    with concurrent.futures.ThreadPoolExecutor() as executor:
        result = tqdm(executor.map(slide_info, path_list), total=len(path_list))
        info_df = pd.DataFrame(
            filter(lambda x: x is not None, result),  # type: ignore
            columns=["name", "r_p10", "g_p10", "b_p10", "r_min", "g_min", "b_min"],  # type: ignore
        )
